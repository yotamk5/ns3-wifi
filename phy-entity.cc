                                ...
                                Time rxDuration)
{
    ...
    case WifiPhyState::IDLE:
        if (m_wifiPhy->m_currentEvent)
        {
            // Two preambles arrived at the exact same simulation time on a shared
            // spectrum channel. Drop the second one as if the PHY were busy.
            NS_LOG_DEBUG("Drop packet because of concurrent preamble on shared channel");
            DropPreambleEvent(ppdu, BUSY_DECODING_PREAMBLE, endRx);
        }
        else
        {
